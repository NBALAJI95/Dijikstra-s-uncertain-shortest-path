...
if __name__ == '__main__':
	...
	while True:
		...
		tmp = tmp[0:4]
		#tmp[2] - > mean tmp[3] -> var
		tmp[2] = tmp[2] + 2 * math.sqrt(tmp[3])
		tmp = tmp[0:3]
		print tmp
		edge_wt.append(tmp)
	...
	output = str('\n') + str('DOUBLE PESSIMIST') + str(',') + str('-'.join(path_res)) + str(',') + str(mean) + str(',')  + str(mean_minus_sd) + str(',')  + str(mean_plus_sd) + str(',')  + str(mean_plus_twice_sd) + str(',')  + str(c_squared) + str(',')  + str(dispersion_index) + str(',')  + str(hops)
	fh.write(output)
